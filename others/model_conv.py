...
class Model(tf.keras.Model):
	...
	def init_kth_inter_block(self, k: int, out_units):
		...
		block.add(Dense(256, name='l'+str(k)+'_inter_dense_0', input_shape=(512,), use_bias=True, kernel_initializer=ln_init))
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
		block.add(Dense(256, name='l'+str(k)+'_inter_dense_1', use_bias=True, kernel_initializer=ln_init))
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
		block.add(Dense(256, name='l'+str(k)+'_inter_dense_2', use_bias=True, kernel_initializer=ln_init))
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
		return block
	...
	def init_kth_out_block(self, k: int, out_units):
		...
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
		block.add(Dense(num_units[0], name='l'+str(k)+'_out_dense_1', use_bias=True, kernel_initializer=ln_init))
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
		block.add(Dense(num_units[1], name='l'+str(k)+'_out_dense_2', use_bias=True, kernel_initializer=ln_init))
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
		block.add(Dense(num_units[1], name='l'+str(k)+'_out_dense_3', use_bias=True, kernel_initializer=ln_init))
		block.add(Activation('relu'))
	...
	def init_base_block(self):
		...
		block.add(Conv1D(64, kernel_size=2, strides=2, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(128, kernel_size=3, strides=2, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(128, kernel_size=3, strides=2, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(256, kernel_size=2, strides=2, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(256, kernel_size=4, strides=3, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(512, kernel_size=2, strides=2, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(512, kernel_size=3, strides=2, activation='relu', kernel_initializer=ln_init, use_bias=True))
		block.add(Conv1D(512, kernel_size=7, strides=1, use_bias=True, kernel_initializer=ln_init))
		#block.add(BatchNormalization())
		block.add(Activation('relu'))
	...
	def call(self, input, training=False):
		...
		concat_factors = [self.concat([base_copy] + inter_factors[0:i], axis=1) for i in range(1, 7)]
...
with strategy.scope(): 
	...
	model.compile(optimizer=tf.keras.optimizers.Adam(lr=1e-5), #, clipnorm=5), # how about mommentum? beta1 and beta 2?
				  ...
				  loss_weights={'output_' + str(i+1): i / 21 for i in range(6)},
...
early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=50, verbose=2, restore_best_weights=True)
csv_logger = tf.keras.callbacks.CSVLogger('/data2/public/chonghui_backup/training_history_by_csv_logger.csv', separator=',', 
										  append=False)

history = model.fit(x=X[0:split_idx], y=[y[0:split_idx] for y in Y], epochs=1, batch_size=256, shuffle=False, 
					validation_data=(X[split_idx:], [y[split_idx:] for y in Y]), verbose=1) 
	# use sample_weight !!!!!!!!!!!!
model.summary()

history = model.fit(x=X[0:split_idx], y=[y[0:split_idx] for y in Y], epochs=1000, batch_size=256, shuffle=False, 
					callbacks=[early_stopping, csv_logger], validation_data=(X[split_idx:], [y[split_idx:] for y in Y]))
