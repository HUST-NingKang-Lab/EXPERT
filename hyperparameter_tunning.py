...
class Model(tf.keras.Model):
	...
	def init_kth_inter_block(self, k: int, out_units):
		...
		#block.add(BatchNormalization())
		#block.add(Activation('relu'))
		#block.add(Dense(num_units[1], name='l'+str(k)+'_inter_dense_3', use_bias=True, kernel_initializer=ln_init))
		return block
...
#if True:
	model = Model()
	model.compile(optimizer=tf.keras.optimizers.SGD(lr=1e-4, momentum=0.64, nesterov=True), # how about mommentum? beta1 and beta 2?
...
history = '/data2/public/chonghui_backup/hyperparameter_tunning/warm_up_lr_1e-4_momentum_0.64.csv'
csv_logger = tf.keras.callbacks.CSVLogger(history, separator=',', append=False)

model.fit(x=X[0:split_idx], y=[y[0:split_idx] for y in Y], epochs=5, batch_size=1024,# shuffle=False, 
					callbacks=[csv_logger], validation_data=(X[split_idx:], [y[split_idx:] for y in Y]), verbose=1) 
	# use sample_weight !!!!!!!!!!!!
model.summary()

model.save('/data2/public/chonghui_backup/tmp_onn_model_hp_tunning', save_format='tf')

for i in range(30):
	lr = np.round(np.random.randint(1, 9) * 10 ** (np.random.randint(-4, -1)), 4)
	#m = 0.75 + np.round(np.random.uniform(-1, 1, 1)[0] / 4, 2)
	m = 0.9
	with strategy.scope():
		model = tf.keras.models.load_model('/data2/public/chonghui_backup/tmp_onn_model_hp_tunning')
	...
	history = '/data2/public/chonghui_backup/hyperparameter_tunning/SGD_lr_{}_momentum_{}.csv'.format(lr, np.round(m, 2))
	csv_logger = tf.keras.callbacks.CSVLogger(history, separator=',', append=False)
	lr_reducer = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=2, min_lr=1e-8, cooldown=0, verbose=2)
	model.fit(x=X[0:split_idx], y=[y[0:split_idx] for y in Y], epochs=30, batch_size=1024, initial_epoch=6, #shuffle=False, 
